    ...
    try {
      if (id) {
        const response = await axios.get(
          process.env.API_HOST + "/api/journal/findAll/" + parseInt(id)
        );
    ...
    <View style={styles.body}>
      ...
      <ScrollView>
        <View style={styles.header}>
          <Text style={styles.subHeading}>Keep{`\n`}Your Mind Clear</Text>
        </View>
        <View style={styles.journals}>
          {journalData.map((entry) => (
            <View style={styles.journalCard} key={entry.entry_id}>
              ...
              >
                <Text style={styles.journalTitleText}>{entry.title}</Text>
              </TouchableOpacity>
            </View>
          ))}
        </View>
      </ScrollView>
      <TouchableOpacity onPress={() => router.push("./journal/addJournal")}>
        <FloatingButton />
      </TouchableOpacity>
    </View>
  );
