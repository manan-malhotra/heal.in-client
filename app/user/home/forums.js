  ...
  return (
    <>
      <Stack.Screen
        options={{
          headerTitle:
            user.role == "RESPONDER" || user.role == "MODERATOR"
              ? "Respond to QnA"
              : "Forums",
        }}
      />
        ...
        <ScrollView>
          ...
          {forumData.length > 0 &&
            forumData.map((question) => (
              <View style={styles.card} key={question.public_qna_id}>
                ...
                <View style={styles.row}>
                  <View style={styles.cardDetailsLeft}>
                    ...
                    <View style={styles.cardDateSection}>
                      <Text style={styles.cardDate}>
                        {formatDate(question.added_date)}
                      </Text>
                    </View>
                  </View>
                  <View style={styles.cardDetailsRight}>
                    {user.role == "USER" && (
                      <>
                        {question.user_id.user_id != user.userId && (
                          <Pressable
                            onPress={() => {
                              setModalVisible(true);
                              setReportIndex(question.public_qna_id);
                            }}
                          >
                            <Icon
                ...
                {question.comments.length > 0 ? (
                  <>
                    <Pressable
                      onPress={() => {
                        handleCommentId(question.public_qna_id);
                      }}
                    >
                      <Text style={styles.commentText}>
                        {commentVisibleId.includes(question.public_qna_id)
                          ? "Hide Comments"
                          : "Show Comments"}
                      </Text>
                    </Pressable>
                      ...
                      <>
                        {question.comments.map((comment) => (
                          <View
                            ...
                            key={comment.comment_id + question.public_qna_id}
                          >
                            <View style={styles.row}>
                              <View style={styles.commentDetailsLeft}>
                              ...
                              <View style={styles.commentDetailsRight}>
                                <View style={styles.commentDateSection}>
                  ...
                  <>
                    ...
                    <View style={styles.newComment}>
                      <TextInput
                        ...
                        onChangeText={(text) => {
                          setComment(question.public_qna_id, text);
                        }}
                        value={comments[question.public_qna_id]}
                      />
                      <Pressable
                        style={styles.commentButton}
                        onPress={() => {
                          handleCommentSend(question.public_qna_id);
                        }}
                      >
                        <Icon
                          name="send"
